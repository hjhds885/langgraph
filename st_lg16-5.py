...
# Move PyTorch imports lower down
#Streamlit のファイルウォッチャーとの競合を避けるための一般的な回避策です。
import torch
import torchaudio
import torchvision
...
def initialize_models(selected_model_name: str):
    ...
    model_definitions = {
        ...
        "gpt-4.1": {"provider": "openai", "temperature": 0, "cost_input": 2.0, "cost_cached_input": 0.5, "cost_output": 8.0, "input_max": 1047576},
        # "gpt-4o": {"provider": "openai", "temperature": 0, "cost_input": 2.5, "cost_cached_input": 1.25, "cost_output": 10, "input_max": 128000},
        # "o1": {"provider": "openai", "temperature": 0, "cost_input": 15, "cost_cached_input": 7.5, "cost_output": 60, "input_max": 200000},
        # "ChatGPT-4o": {"provider": "openai", "temperature": 0, "cost_input": 5, "cost_cached_input": 0, "cost_output": 15, "input_max": 128000},
        # "o3": {"provider": "openai", "temperature": 0, "cost_input": 10, "cost_cached_input": 2.5, "cost_output": 40, "input_max": 200000},
        # "o1-mini": {"provider": "openai", "temperature": 0, "cost_input": 1.1, "cost_cached_input": 0.55, "cost_output": 4.4, "input_max": 128000}, # img NG
        # "o3-mini": {"provider": "openai", "temperature": 0, "cost_input": 1.1, "cost_cached_input": 0.55, "cost_output": 4.4, "input_max": 200000}, # img NG
        # "claude-3-7-sonnet": {"provider": "anthropic", "model_id": "claude-3-7-sonnet-latest", "temperature": 0, "cost_input": 3, "cost_cached_input": 0, "cost_output": 15, "input_max": 200000},
        # "claude-3-5-sonnet": {"provider": "anthropic", "model_id": "claude-3-5-sonnet-latest", "temperature": 0, "cost_input": 3, "cost_cached_input": 0, "cost_output": 15, "input_max": 200000},
        # "claude-3-5-haiku": {"provider": "anthropic", "model_id": "claude-3-5-haiku-latest", "temperature": 0, "cost_input": 0.8, "cost_cached_input": 0, "cost_output": 4, "input_max": 200000},
        # "claude-3-haiku": {"provider": "anthropic", "model_id": "claude-3-haiku-latest", "temperature": 0, "cost_input": 0.8, "cost_cached_input": 0, "cost_output": 4, "input_max": 200000},
        # "neva_22b": {"provider": "nvidia", "model_id": "nvidia/neva-22b", "temperature": 0, "cost_input": 0, "cost_cached_input": 0, "cost_output": 0, "input_max": 158000}, # 日本語暴走
        # "microsoft/phi-4-multimodal-instruct": {"provider": "nvidia", "temperature": 0, "cost_input": 0, "cost_cached_input": 0, "cost_output": 0, "input_max": 128000}, # 日本語良くない
        "c4ai-aya-vision-32b": {"provider": "cohere", "temperature": 0, "cost_input": 0, "cost_cached_input": 0, "cost_output": 0, "input_max": 16000}, # ツール非対応
        # "meta/llama-4-maverick-17b-128e-instruct": {"provider": "nvidia", "temperature": 0, "cost_input": 0, "cost_cached_input": 0, "cost_output": 0, "input_max": 1000000}, # ツール非対応 (NVIDIA版)
    }
    """
    ...
    for segment in split_response:
        if segment.strip():
            ...
            try:
                ...
                # --- ▼▼▼ gTTS と Pydub 処理を別スレッドで実行 ▼▼▼ ---
                loop = asyncio.get_running_loop()
                # gTTS (I/O バウンド)
                # tts = gTTS(cleaned_segment, lang="ja") # 同期処理
                tts = await loop.run_in_executor(None, lambda: gTTS(cleaned_segment, lang="ja")) # 非同期処理
                audio_buffer = BytesIO()
                await loop.run_in_executor(None, tts.write_to_fp, audio_buffer) # 非同期処理
                audio_buffer.seek(0) # これはメインスレッドでOK
                ...
                await loop.run_in_executor(None, lambda: audio.export(output_buffer, format="mp3")) # 非同期処理
                output_buffer.seek(0) # これはメインスレッドでOK
...
async def whis_seg2(audio_segment: AudioSegment):
    ...
    # --- ▼▼▼ Whisper 処理を別スレッドで実行 ▼▼▼ ---
    loop = asyncio.get_running_loop()
    # result = whisper_model.transcribe(audio_data, language="ja") # 同期処理
    #result = await loop.run_in_executor(None, whisper_model.transcribe, audio_data, "ja") # 非同期処理
    #result = await loop.run_in_executor(None, whisper_model.transcribe, audio_data, language="ja") 
    result = await loop.run_in_executor(None, lambda: whisper_model.transcribe(audio_data, language="ja")) # lambda を使ってキーワード引数を渡す
    # --- ▲▲▲ Whisper 処理を別スレッドで実行 ▲▲▲ ---
    return result['text']

#def transcribe(audio_segment: AudioSegment, debug: bool = False) -> str:
async def transcribe(audio_segment: AudioSegment, debug: bool = False) -> str: # 非同期に変更
    answer2 ="（休止中）"
    # ステレオの場合、モノラルに変換
    ...
    answer2 = await whis_seg2(audio_segment) # ★ await を追加
    return answer2
...
async def process_audio(audio_data_bytes, sample_rate, sound_chunk):
    ...
    if len(sound_chunk) > 0:
        ...
        answer2 = await transcribe(sound_chunk) # await を追加
    return answer2
...
async def process_audio_loop_with_silence_detection(
    ...
    音声認識を行い、結果を st.session_state に格納して状態を更新する。
    テキスト入力やボタン入力も検知して返す。
    """
    audio_buffer = []
    ...
    #while True:
    # listening 状態の間だけループ
    while st.session_state.get("app_state", "idle") == "listening":
        ...
        if silence_detected and audio_buffer:
            ...
            try:
                ...
                #answer2 = asyncio.run(process_audio(audio_data, st.session_state.frame_sample_rate, sound_chunk)) # await を asyncio.run に変更
                ##########################################################
                #text_output.write(f"認識結果: {answer}")
                #おかしな回答を除去
                # テキスト出力が空、または空白である場合もチェック
                phrases = (
                    "ありがとう", "ご視聴ありがとうございました",
                    "お疲れ様", "んんんんんん", 
                    "by H.","スタッフさんのお話を",
                    "いいえ- いいえ- いいえ-",
                    "ごちそうさまでした","チャンネル登録を",
                ...
                if len(answer2) < 5:
                    ...
                    #return answer2
                    print("[Whis_seg]",answer2)
                    st.session_state.recognized_text = answer2 # 結果を保存
                    st.session_state.app_state = "processing_audio" # ★ 状態を更新
                    return # ループを抜ける (st.rerun は呼ばない) 
        ...
        # 処理負荷を抑えるために短い遅延を挿入
        #time.sleep(0.1)
        await asyncio.sleep(0.1) # time.sleep を asyncio.sleep に変更
...
async def main():
    ...
    with st.sidebar:
        ...
        webrtc_ctx = webrtc_streamer(
            ...
            media_stream_constraints={"video": True, "audio": True},  #False
    ...
    for i in range(max_retries):
        ...
        warning_message = []
        if not video_ready: warning_message.append("カメラorマイク")
    ...
    if not webrtc_ctx.state.playing : #or not webrtc_ctx_audio.state.playing
        ...
        #st19-7up(1).pyではデプロイしても不具合がないが、st_lg15.pyにおいて、
        # streamlit cloudにデプロイすると、”Webカメラとマイクを開始してください。”
        # の表示が続いたまま次の処理にすすまない不具合があります。
        # ローカルPCでは、問題なく、次の処理に進みます。
        # この不具合の原因と対処方法を教えてください。
        return
    # --- 初期化 ---
    if "app_state" not in st.session_state:
        st.session_state.app_state = "idle" # 状態管理を追加
    ...
    #amp_indicator = st.sidebar.empty() # 音声振幅表示用 (ループ外に移動)
    ...
    if st.session_state.selected_model_name is None or selected_model_name != st.session_state.selected_model_name:
        ...
        # --- ▼▼▼ 選択されたモデルのみをロード ▼▼▼ ---
        with st.spinner(f"モデル '{selected_model_name}' をロード中..."):
    ...
    with button_container:
        ...
        for i, (label, query) in enumerate(button_definitions):
            if cols[i % 7].button(label, key=f"button_{i}"):
                button_input = query
                st.session_state.recognized_text = query # ボタン入力も recognized_text に
                st.session_state.app_state = "processing_button" # 状態変更
    ...
    #     recognized_text = process_audio_loop_with_silence_detection( 
    #         frames_deque_lock,
    #         frames_deque,
    #         sound_chunk,
    #         amp_indicator,
    ...
    if current_state == "idle":
        status_indicator.write("🎤 話してください...又はテキストを入力/ボタンを押してください。")
        # アイドル状態なら自動的にリスニング状態へ移行
        st.session_state.app_state = "listening"
        # ここで rerun する必要はない。次のスクリプト実行で listening 状態の処理に入る。

    # 2. 音声入力受付中
    elif current_state == "listening":
        status_indicator.write("🎤 聞き取り中...")
        sound_chunk = pydub.AudioSegment.empty()
        # 音声入力ループを実行。この関数内で認識完了時に app_state が "processing_audio" に変わる
        # ボタンやテキスト入力があった場合もこの関数内で検知して返す
        recognized_text_or_input = await process_audio_loop_with_silence_detection(
            frames_deque_lock,
            frames_deque,
            sound_chunk,
            amp_indicator,
            status_indicator,
            button_input, # ボタン入力があれば渡す
            prompt, # テキスト入力があれば渡す
            memory_use,memory_alt,memory_ok,
        )
        # process_audio_loop_with_silence_detection が状態を変えたか、
        # またはテキスト/ボタン入力を返した場合、Streamlitが自動で再実行し、
        # 次の processing_* 状態に進むはず

    # 3. LLM処理中 (音声/テキスト/ボタン共通)
    elif current_state.startswith("processing"):
        final_prompt = st.session_state.recognized_text
        if final_prompt:
            st.session_state.recognized_text = None # 処理するのでクリア
            status_indicator.write("🧠 考え中...")
            # --- ここから LLM 呼び出しまでの処理 (st_lg15-1.py と同様) ---
            ...
            if uploaded_file is None and ("画像" in final_prompt or "カメラ" in final_prompt or "画面" in final_prompt):
                ...
                if cap is not None :
                    ...
                    st.warning("カメラフレームを取得できませんでした。WebRTC接続を確認してください。")
            ...
            if graph_instance:
                with history_container:
                    with st.chat_message("assistant",avatar="🛸"):
                        message_placeholder = st.empty() if st.session_state.output_method == "テキスト" else None
                        full_response = ""
                        tool_calls_info = []
                        ...
                        try:
                            # --- Streaming Execution ---
                            stream_mode = "messages" if is_react_agent else "values"
                            spinner_text = f"🧠 {exec_mode} 実行中..."
                            with st.spinner(spinner_text):
                                ...
                                for event_data in events:
                                    # ReAct の場合は event_data がメッセージリスト
                                    # LangGraph の場合は event_data が State 辞書
                                    last_message = None
                                    if is_react_agent and isinstance(event_data, list) and event_data:
                                        last_message = event_data[-1]
                                    elif not is_react_agent and isinstance(event_data, dict) and "messages" in event_data:
                                        current_graph_messages = event_data["messages"]
                                        if current_graph_messages:
                                            last_message = current_graph_messages[-1]

                                    if isinstance(last_message, AIMessage):
                                        if getattr(last_message, 'tool_calls', None):
                                            tool_calls_info.append(str(getattr(last_message, 'tool_calls')))
                                            ...
                                            final_ai_message = last_message # 最後の有効なAIメッセージを保持

                            # --- ストリーミング完了後 ---
                            if message_placeholder:
                                message_placeholder.markdown(full_response) # カーソルなしで最終表示

                            # --- 応答処理 ---
                            if final_ai_message:
                                st.session_state.messages.append(final_ai_message)
                                st.session_state.llm_response = full_response # 音声出力用に保存
                                st.session_state.app_state = "displaying" # 表示状態へ
                                st.rerun() # UI更新と次の状態遷移
                            else:
                                # フォールバック処理
                                st.warning("最終応答をストリームから取得できませんでした。状態を確認します。")
                                try:
                                    snapshot = graph_instance.get_state(config)
                                    # ... (状態からの復元処理 - st_lg15-1.py と同様) ...
                                    # 復元成功時:
                                    # st.session_state.app_state = "displaying"
                                    # st.rerun()
                                    # 復元失敗時:
                                    if message_placeholder: message_placeholder.markdown("エラーまたは応答なし。")
                                    st.session_state.messages.append(AIMessage(content="応答を取得できませんでした。"))
                                    st.session_state.app_state = "idle"
                                    st.rerun()
                                except Exception as state_e:
                                    # ... (エラー処理 - st_lg15-1.py と同様) ...
                                    st.session_state.app_state = "idle"
                                    st.rerun()

                        except Exception as e:
                            # ... (LLM実行エラー処理 - st_lg15-1.py と同様) ...
                            st.session_state.app_state = "idle"
                            st.rerun()
            else:
                # ... (グラフインスタンスなしエラー処理 - st_lg15-1.py と同様) ...
                st.session_state.app_state = "idle"
                st.rerun()
        else:
             # final_prompt が空の場合 (予期せぬケース)
             st.session_state.app_state = "idle"
             # この場合、rerun は不要かもしれないが、念のため入れておく
             st.rerun()

    # 4. 応答表示中
    elif current_state == "displaying":
        status_indicator.empty() # "考え中..." を消す
        # テキスト表示は processing 状態で完了している

        # 音声出力
        if st.session_state.output_method == "音声" and st.session_state.llm_response:
            response_text = st.session_state.llm_response
            st.session_state.llm_response = None # 処理するのでクリア
            with st.spinner("🔊 音声出力中..."):
                 await streaming_text_speak(response_text)

        # 状態をアイドルに戻して次の入力を待つ
        st.session_state.app_state = "idle"
        print("処理終了、アイドル状態へ")
        # 表示完了後、すぐに rerun する必要はない。次のインタラクションを待つ。
        # st.rerun() # 不要
