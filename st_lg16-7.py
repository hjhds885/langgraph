...
async def process_audio_loop_with_silence_detection(
    ...
    while st.session_state.get("app_state", "idle") == "listening":
        ...
        await asyncio.sleep(0.05) # time.sleep を asyncio.sleep に変更し、待機時間を短縮 (0.1 -> 0.05)
...
async def main():
    ...
    for i in range(max_retries):
        ...
        await asyncio.sleep(retry_delay) # ★ time.sleep を asyncio.sleep に変更
    else:
    ...
    whisper_model_size = st.sidebar.selectbox(
        "Whisperモデルサイズ (メモリ/精度トレードオフ):",
        ("tiny", "base", "small"), # "medium", "large" はメモリ消費大
        index=2, # デフォルトは "small"
    ...
    #         st.session_state.whisper_model = whisper.load_model("small")
    if "whisper_model" not in st.session_state or st.session_state.get("whisper_model_size_loaded") != whisper_model_size:
        with st.spinner(f"Whisperモデル ({whisper_model_size}) をロード中..."):
            # 既存のモデルがあれば削除してメモリ解放を試みる
            ...
            st.session_state.whisper_model_size_loaded = whisper_model_size # ロードしたサイズを記録
            gc.collect() # ロード後にも念のため実行
    # --- 情報表示 ---
    ...
    if st.session_state.selected_model_name is None or selected_model_name != st.session_state.selected_model_name:
        ...
        if st.session_state.graph_or_agent is None:
            ...
            # --- ▼▼▼ モデル切り替え後にガベージコレクション ▼▼▼ ---
            gc.collect() # 不要になった古いモデルのリソース解放を期待
            st.rerun()
